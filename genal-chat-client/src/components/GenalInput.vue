            ...
            <div class="message-tool-item">
              <a-upload :show-upload-list="false" :before-upload="beforeImgUpload">
                <div class="message-tool-contant">
                  ...
                  <div class="message-tool-item-text">Picture</div>
                </div>
              </a-upload>
            </div>
          </a-tab-pane>
...
export default class GenalInput extends Vue {
  ...
  throttle(fn: Function, file?: File) {
    ...
    if (nowTime - this.lastTime < 500) {
      return this.$message.error('message sent too frequently!');
    }
    fn(file);
  ...
  preSendMessage() {
    if (!this.text.trim()) {
      this.$message.error('Can\'t send empty message!');
      return;
    }
    if (this.text.length > 220) {
      this.$message.error('message oversized!');
      return;
    }
  ...
  async handleImgUpload(imageFile: File) {
    ...
    if (!isJpgOrPng) {
      return this.$message.error('Please select a picture in jpeg/jpg/png/gif format!');
    }
    const isLt1M = imageFile.size / 1024 / 1024 < 0.5;
    if (!isLt1M) {
      return this.$message.error('Image must be smaller than 500K!');
    }
    let image = new Image();
    let url = window.URL || window.webkitURL;

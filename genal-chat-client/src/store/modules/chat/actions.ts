...
const actions: ActionTree<ChatState, RootState> = {
  ...
  async connectSocket({ commit, state, dispatch, rootState }, callback) {
    ...
    socket.on('joinGroup', async (res: ServerRes) => {
      ...
      if (newUser.userId != user.userId) {
        ...
        return Vue.prototype.$message.info(`${newUser.username}add group${group.groupName}`);
      } else {
        ...
        Vue.prototype.$message.info(`success add in ${group.groupName}`);
    ...
    socket.on('joinGroupSocket', (res: ServerRes) => {
      ...
      if (newUser.userId != user.userId) {
        ...
        return Vue.prototype.$message.info(`${newUser.username} add in group ${group.groupName}`);
      } else {
        if (!state.groupGather[group.groupId]) {
          commit(SET_GROUP_GATHER, group);
        }
  ...
  async handleChatData({ commit, dispatch, state, rootState }, payload) {
    ...
    if (!activeRoom) {
      // 更新完数据没有默认activeRoom设置群为'Public Room'

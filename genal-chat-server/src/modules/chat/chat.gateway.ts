...
export class ChatGateway {
  ...
  async addGroup(@ConnectedSocket() client: Socket, @MessageBody() data: Group):Promise<any> {
    ...
    if(isUser) {
      ...
      if (isHaveGroup) {
        this.server.to(data.userId).emit('addGroup', { code: RCode.FAIL, msg: 'group name exists', data: isHaveGroup });
        return;
      }
      ...
      this.server.to(group.groupId).emit('addGroup', { code: RCode.OK, msg: `creat ${data.groupName} successfully`, data: group });
      this.getActiveGroupUser();
    } else{
      this.server.to(data.userId).emit('addGroup', { code: RCode.FAIL, msg: `you dont have right to create` });
    }
  }
    ...
    if(isUser) {
      ...
      if (group && user) {
        ...
        this.server.to(group.groupId).emit('joinGroup', {
          code: RCode.OK,
          msg: `${user.username}join group${group.groupName}`,
          data: res
        });
        this.getActiveGroupUser();
      } else {
        this.server.to(data.userId).emit('joinGroup', { code: RCode.FAIL, msg: 'entering failed', data: '' });
      }
    } else {
      this.server.to(data.userId).emit('joinGroup', { code: RCode.FAIL, msg: 'You are not eligible to join the group'});
    }
  }
  ...
  async joinGroupSocket(@ConnectedSocket() client: Socket, @MessageBody() data: GroupMap):Promise<any> {
    ...
    if(group && user) {
      client.join(group.groupId);
      const res = { group: group, user: user};
      this.server.to(group.groupId).emit('joinGroupSocket', {code: RCode.OK, msg:`${user.username}get in the group${group.groupName}`, data: res});
    } else {
      this.server.to(data.userId).emit('joinGroupSocket', {code:RCode.FAIL, msg:'get in failed', data:''});
    }
  }
  ...
  async sendGroupMessage(@MessageBody() data: GroupMessageDto):Promise<any> {
    ...
    if(isUser) {
      ...
      if(!userGroupMap || !data.groupId) {
        this.server.to(data.userId).emit('groupMessage',{code:RCode.FAIL, msg:'Group message sending error', data: ''});
        return;
      } 
      ...
      this.server.to(data.userId).emit('groupMessage', {code: RCode.FAIL, msg:'You are not qualified to send messages' });
    }
  }
    ...
    if(isUser) {
      if (data.friendId && data.userId) {
        if (data.userId === data.friendId) {
          this.server.to(data.userId).emit('addFriend', { code: RCode.FAIL, msg: 'Cant add yourself as a friend', data: '' });
          return;
        }
        ...
        if (relation1 || relation2) {
          this.server.to(data.userId).emit('addFriend', { code: RCode.FAIL, msg: 'already is your friend', data: data });
          return;
        }
        ...
        const user = await this.userRepository.findOne({userId: data.userId});
        if (!friend) {
          this.server.to(data.userId).emit('addFriend', { code: RCode.FAIL, msg: 'person does not exist', data: '' });
          return;
        }
        ...
        this.server.to(data.userId).emit('addFriend', { code: RCode.OK, msg: `add user ${friend.username} success`, data: friend });
        this.server.to(data.friendId).emit('addFriend', { code: RCode.OK, msg: `${user.username}add you as a friend`, data: user });
      }
    } else {
      this.server.to(data.userId).emit('addFriend', {code: RCode.FAIL, msg:'You are not eligible to add friends' });
    }
  }
  ...
  async joinFriend(@ConnectedSocket() client: Socket, @MessageBody() data: UserMap):Promise<any> {
    if(data.friendId && data.userId) {
      ...
      if(relation) {
        client.join(roomId);
        this.server.to(data.userId).emit('joinFriendSocket',{ code:RCode.OK, msg:'Entering the private chat socket successfully', data: relation });
      } 
    }
  }
  ...
  async friendMessage(@ConnectedSocket() client: Socket, @MessageBody() data: FriendMessageDto):Promise<any> {
    ...
    if(isUser) {
      ...
      this.server.to(data.userId).emit('friendMessage', {code: RCode.FAIL, msg:'You are not qualified to send messages', data});
    }
  }
  ...
  async getAllData(@ConnectedSocket() client: Socket,  @MessageBody() user: User):Promise<any> {
    ...
    if(isUser) {
      ...
      this.server.to(user.userId).emit('chatData', {code:RCode.OK, msg: 'Get chat data successfully', data: {
        groupData: groupArr,
  ...
  async exitGroup(@ConnectedSocket() client: Socket,  @MessageBody() groupMap: GroupMap):Promise<any> {
    if(groupMap.groupId === this.defaultGroup) {
      return this.server.to(groupMap.userId).emit('exitGroup',{code: RCode.FAIL, msg: 'The default group is non-returnable'});
    }
    ...
    if(user && group && map) {
      ...
      this.server.to(groupMap.userId).emit('exitGroup',{code: RCode.OK, msg: 'Successfully exit from the group', data: groupMap});
      return this.getActiveGroupUser();
    }
    this.server.to(groupMap.userId).emit('exitGroup',{code: RCode.FAIL, msg: 'Failed to exit from the group'});
  }
  ...
  async exitFriend(@ConnectedSocket() client: Socket,  @MessageBody() userMap: UserMap):Promise<any> {
    ...
    if(user && friend && map1 && map2) {
      ...
      return this.server.to(userMap.userId).emit('exitFriend',{code: RCode.OK, msg: 'Delete friends successfully', data: userMap});
    }
    this.server.to(userMap.userId).emit('exitFriend',{code: RCode.FAIL, msg: 'Failed to delete friend'});
  }

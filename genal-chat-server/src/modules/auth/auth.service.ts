...
export class AuthService {
  ...
  async login(data: User): Promise<any> {
    ...
    if(!user) {
      return {code: 1 , msg:'wrong password', data: ''};
    }
    if(!passwordVerify(data.password) || !nameVerify(data.username)) {
      return {code: RCode.FAIL, msg:'signup failed', data: '' };
    }
  ...
  async register(user: User): Promise<any> {
    ...
    if(isHave.length) {
      return {code: RCode.FAIL, msg:'username exists', data: '' };
    }
    if(!passwordVerify(user.password) || !nameVerify(user.username)) {
      return {code: RCode.FAIL, msg:'sign up failed', data: '' };
    }
